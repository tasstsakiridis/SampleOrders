trigger SAPInterfacedData on SAP_Interfaced_Data__c (before update) {
    ...
    for(SAP_Interfaced_Data__c sap : Trigger.new) {
        if (sap.Approval_Status__c == 'Approved' && sap.RecordTypeId == rtSampleOrder && sap.Brand_Manager_Emailed__c == false) {
    ...
    if (sampleOrderIds.size() > 0) {
        ...
        if (brandMap.size() > 0) {
            ...
            String brandQueryStr = 'SELECT Id, Email, Manages_Brand__c FROM User WHERE Manages_Brand__c INCLUDES (';
